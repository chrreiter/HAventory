...
(> is correct as it stands, not defects. Both promoted items are now **resolved**: the
> suite stops every interval it still holds immediately before jsdom goes away
> (`test.setup.ts`), so `register.ts`'s recheck — correct in production, and untouched —
> cannot tick into a torn-down window (**74**); and `makeItem` stamps fixtures from a
> constant instead of the wall clock, so the default `updated_at` tie-break decides their
> order on every run, with anonymous ids counted rather than read off `Date.now()` (**73**).
> Their rows are removed below. The same pass truthed up item **46**: its row claimed
