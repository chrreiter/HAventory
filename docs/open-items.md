...
> Item **58** (the sidebar panel) is delivered: PR-2 registers `haventory-panel` at
> `/haventory` through `panel_custom` on the same module URL both card loaders receive,
> converging remove-then-register so a reload cannot hit `Overwriting panel haventory`,
> under a `sidebar_panel_enabled` option (default on) that applies live through the
> existing update listener. The phacc suite asserts the real registration at the 2026.6.0
> floor. Its row is removed below.
