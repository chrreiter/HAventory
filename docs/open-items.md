...
- A **card UI consistency review (2026-07-26)**, run against the revamped card on
  `claude/ui-revamp-implementation-8fl22q` at 1440×900 and 375×812, supplied items 33–35.
  Everything else it found was fixed in that branch; these three are here because each needs
  a backend or contract change (33, 35) or was deferred on effort (34). All three were
  deferred past v1.0 by the owner, and each is measured against the running container rather
  than read off the source alone.
