...
| 58 | **Sidebar panel — HAventory's place in the redesigned frontend.** HA's new Overview (the built-in `home` panel, default for new installations since the 2026.6 dashboard overhaul; verified against 2026.7.4) has **no card surface at all**: its editor offers favorites, fixed summaries and navigation *shortcuts* only, and its config is per-user (`frontend/set_user_data`), so an integration can inject nothing server-side. The one first-class integration point is a navigation path — so register a `/haventory` sidebar panel (`panel_custom.async_register_panel`, API verified identical at the 2026.6.0 floor and current stable) rendering the card's extended view (`hv-full-view` gains an `embedded` variant) full-page from the existing bundle. HAventory then appears in the sidebar on install, and the Overview's "Add shortcut" picker — which enumerates `hass.panels` — can pin it to the Overview page. An options-flow boolean (default on) registers/removes the panel live through the existing update listener; per-user hiding already exists natively in HA's sidebar edit mode. Full plan, feasibility evidence and staged Opus 5 implementation prompts: [`sidebar-panel.md`](sidebar-panel.md). | owner report 2026-08-01 (fresh-install test: card not addable to the new Overview) | Medium (user-facing discoverability) | M |
| 59 | **The sidebar panel hosts two of `hv-full-view`'s surfaces; the rest have no handler there.** PR-1 of item 58 gives `haventory-panel` the two host-owned surfaces the card host owns — the column picker and the export download — as the plan specifies. But `hv-full-view` raises more than that on its own, independently of the ⋮ menu its host supplies: the item editor's Delete button (`request-delete`), the sidebar's "+" beside Categories and Tags (`menu-action` `organize`), and the empty state's "Import backup" offer (`import`) — the last one being what a fresh install sees first, which is exactly the audience the panel exists for. In the card those land on `hv-card-shell`, which owns the confirm, the organize dialog and the import sheet; the panel has no shell, so all three are visible controls that do nothing. Diagnostics is not in this list only because the panel's menu omits it. Fixing it means lifting those surfaces out of `hv-card-shell` into something both hosts use — the same extraction `host-surfaces.ts` starts — rather than a second copy in the panel. Decide before PR-2 turns the panel on. | PR-1 of item 58 | Medium (user-facing: dead controls) | M |
