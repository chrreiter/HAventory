...
> is correct as it stands, not defects. Item **75** is now resolved: the store subscribes to
...
> the card then behaves as it did before, holding the areas it fetched at boot. Its row is
> removed below. The same pass truthed up item **46**: its row claimed
