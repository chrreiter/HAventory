...
| 12 | ~~**Unify `hv-category-browser` + `hv-tag-browser`**~~ **Done (WP4.1).** Both are superseded by the tabbed `hv-organize-dialog`, which also gains rename, merge and guarded removal. The old browsers remain only behind `ui: legacy`. Issue #87 can be closed. | #84, issue #87 | Low | M |
