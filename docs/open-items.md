...
> Item **34** (the desktop filter chips announce no state) is resolved. `hv-filter-panel`
> settled on **one vocabulary for the whole component**: `aria-pressed` on every filter
> toggle — the seven chips the row named (category, tag, and the five "Show only" facets),
> the tag chips a filter carries that no item does, and the three checkbox-drawn rows
> ("Include sub-locations", "Low stock first", and the "Show only" facets in the phone
> branch), which gave up `role="checkbox"`/`aria-checked` so one facet cannot announce as a
> checkbox in the sheet and a toggle on the panel. That is the idiom both app bars' stat
> pills and the sidebar's facet rows already use, so a filter now reads the same wherever
> the card switches one on. Two things deliberately keep their own words: the tag-match and
> sort-direction pickers stay `role="radio"`/`aria-checked` (they are radiogroups, not
> toggles), and the location chip stays a disclosure with `aria-expanded`, its label naming
...
> location does **not** clear the tree. `Repository.update_location` compares the request
> against `loc.area_id` — the location's **own stored** area, which is null everywhere but
> the root — so `area_change_requested` is false and the save is a no-op. Only a location
> that stores an area can clear one, and only an explicit pick reassigns. The preview says
> what actually happens: `Assigns ⟨area⟩ to the whole ⟨root⟩ tree, N locations. The area is
> stored on ⟨root⟩, not on this one.` / `Removes the area from the whole ⟨root⟩ tree, N
> locations.` / for the no-op case on a nested location, `Inherits ⟨area⟩ from its location
> tree.` — which also closes the gap the relabel (#126) left, where "Inherit from location
> tree" named the source but never the area. `areaChangePreview` (`ui/area.ts`) follows the
> parent **as picked in the dialog**, not as recorded: both halves go out in one
> `location/update` and the backend propagates after the move, so a simultaneous re-parent
> lands the area on the new tree's root. Cosmetic half done with it: the area field is
