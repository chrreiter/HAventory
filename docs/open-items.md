...
- Status verified against the working tree at `main` @ WP4 (`390cba6`), then reconciled
  ...
  sidebar panel's PR-2 (#160) landed, and re-checked against `main` @ `9180d8f` in the
  **#162 collection pass** (2026-08-02).
...
> is correct as it stands, not defects. The same pass truthed up item **46**: its row claimed
...
| 74 | **`register.ts`'s recheck interval outlives the test environment and fails the whole vitest run.** `defineCardElement` starts a 250 ms `setInterval` that re-asserts the definition for 15 s, because HA swaps `window.customElements` during boot and a card loaded through `add_extra_js_url` can land either side of the swap. Every spec that imports the card module starts one, and any spec file that finishes in under 15 s leaves ticks scheduled past jsdom teardown, where `customElements` no longer exists: vitest reports `ReferenceError: customElements is not defined` as an **uncaught exception** and exits 1 with every one of the 940 tests passing. Reproduced on PR #162 — the Node 24 leg failed, the Node 22 leg passed the identical commit, and a re-run of the failed leg passed. The production behaviour is right and must not be weakened; the fix belongs in the test setup (`src/test.setup.ts`, which today only polyfills `ResizeObserver`) — stop the interval on teardown, or export a disposer the spec files call. **Pre-v1.0 because the gate is a required check:** `frontend (22)` and `frontend (24)` are two of the ten status checks item 7's ruleset enforces on `main`, so a run that fails this way blocks every merge, the release PR included. | item 38 stage 4 (CI, PR #162) | Medium (CI reliability) | S |
...
| 75 | **The card's HA area cache is fetched once and never refreshed.** `Store.refreshAreas()` has exactly one caller — `Store.init()` (`store/store.ts:298`) — and the store is built once per element instance, from the `hass` setter in `index.ts` / `haventory-panel.ts`. Locations refetch on their own events and after any mutation; areas never do, and even `reloadAll()` ("refresh every derived cache") skips them. So an area renamed, created or deleted in HA settings does not reach an open dashboard until a full page reload — and the sidebar panel is long-lived, surviving navigation. Harmless while areas showed up in two `<select>`s; item 38 put the name on every list row, table cell, detail-sheet crumb, breadcrumb, filter chip and tree header, and a deleted area now shows as its raw id everywhere at once (`areaNameById`'s deliberate fallback — a blank would read as "no area", the one thing it is not). Two shapes, both S: refetch areas alongside the location refresh, or subscribe to HA's `area_registry_updated` event through the connection the store already has. | #162 collection pass 2026-08-02 | Low–Med (stale UI) | S |
...
- The **item 38 area work (2026-08-01)** — four staged commits on one branch, designed in
  ...
  made visible by printing the area everywhere — and moved 73 and 74 to pre-v1.0. The PR
