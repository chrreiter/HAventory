...
- `haventory/stats`
  ...
  - `inspection_due_count` is the number of items whose `inspection_date` is **on or before** today in UTC — the same population as `inspection_overdue_count` plus the items due today, so it is never smaller than it. *Due* includes today and *overdue* does not, the same distinction `reminder_due_count` draws. No `item/list` filter matches it: `inspection_overdue_only` is strict. Calendar-derived with the same no-event caveat as the counts above.
