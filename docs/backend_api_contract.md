...
- `haventory/item/list`
  ...
  - **`filter.area_id` is refused unless it names an area**: `null` (or an omitted key) is no area filter, a string with non-whitespace in it is the area, and everything else — a blank string, a number, a list — answers `validation_error` ("area_id must be a non-empty string or null"). It is the one filter key checked on its value here, because it is the one applied by an index alone: an item's area is resolved by walking its location tree, so the item-by-item pass behind the index carries no area predicate and a value naming no area would narrow nothing. A well-formed id no location resolves to is a normal empty page. `haventory/subscribe` refuses the same values.
