...
- Serving an attachment — `GET /api/haventory/media/{item_id}/{attachment_id}`
  ...
  - The tile lives at `<attachment_id>.thumb.webp` beside the original. The orphan sweep is told about it, so it survives; deleting the attachment deletes it. It holds no metadata, appears in no export, and can be deleted from disk at any time — the next request writes it again.
