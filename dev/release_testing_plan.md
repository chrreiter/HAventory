...
| D8 | Integration **rollback** N+1 → N (ENV-B only) | Newer-schema data is **refused loudly**: setup fails with `ConfigEntryError` naming both versions and the store file is left byte-identical — never migrated down, never silently relabeled (decided; item 25 fixed by #120). Rolling back *across* the collapse is the one case that is not a refusal: a pre-collapse build reads the adopted v1 store and steps it back up — lossily in exactly one way, because its status backfill knows only the built-in three slugs, so an item on a household-defined status comes back as `ok` (the definition itself survives). The pre-upgrade export is the lossless way back | ✅ |
