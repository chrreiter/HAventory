...
| A4 | Attempt a second config entry | HAventory is absent from the "Add integration" picker while an entry exists; a flow initiated outside the picker aborts with "Already configured. Only a single configuration is possible." No duplicate storage or resource | |
